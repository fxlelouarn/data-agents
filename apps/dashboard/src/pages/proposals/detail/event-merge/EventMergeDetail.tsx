...
const EventMergeDetail: React.FC<EventMergeDetailProps> = ({ proposal }) => {
  ...
  // Utiliser les éditions stockées ou celles récupérées via API
  const keepEventEditions = storedKeepEditions || keepEventData?.editions?.map((e: any) => ({ id: e.id, year: e.year })) || []
  const duplicateEventEditions = storedDuplicateEditions || duplicateEventData?.editions?.map((e: any) => ({ id: e.id, year: e.year })) || []

  // Trier les éditions par année décroissante
  const sortedKeepEditions = (keepEventEditions as EditionInfo[])
    .slice()
    .sort((a, b) => b.year - a.year)
  const sortedDuplicateEditions = (duplicateEventEditions as EditionInfo[])
    .slice()
    .sort((a, b) => b.year - a.year)
            ...
            <CardContent>
              ...
              {isLoadingEditions ? (
                ...
                <Stack direction="row" spacing={0.5} flexWrap="wrap" useFlexGap sx={{ gap: 0.5 }}>
                  {/* Éditions existantes */}
                  {sortedKeepEditions.map((edition: EditionInfo) => (
                    <Chip
                      key={edition.id}
                      label={edition.year}
                      size="small"
          ...
          >
            <CardContent>
              ...
              {isLoadingEditions ? (
                ...
                <Stack direction="row" spacing={0.5} flexWrap="wrap" useFlexGap sx={{ gap: 0.5 }}>
                  {sortedDuplicateEditions.map((edition: EditionInfo) => (
                    <Chip
                      key={edition.id}
                      label={edition.year}
                      size="small"
