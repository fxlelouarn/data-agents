# État du refactoring - 12 novembre 2025

**Date** : 2025-11-12  
**Statut global** : ✅ PHASE 2 COMPLÈTE 🎉

---

## 📊 Vue d'ensemble

| Composant | Statut | Hook utilisé | Notes |
|-----------|--------|--------------|-------|
| `GroupedProposalDetailBase` | ✅ Migré | `useProposalEditor` (mode groupé) | Prêt pour nettoyage |
| `ProposalDetailBase` | ✅ Migré | `useProposalEditor` (mode simple) | Prêt pour nettoyage |

---

## ✅ PHASE 2 : GroupedProposalDetailBase (TERMINÉ)
...
if (!isGroupReturn(editorResult)) {
  throw new Error('Mode groupé requis')
}
...
#### 2. Handlers adaptés (lignes 222-291)
...
const handleFieldModify = (fieldName: string, newValue: any) => {
  ...
  // Compatibilité (à supprimer en PHASE 3)
...
const handleRaceFieldModify = (raceId: string, fieldName: string, newValue: any) => {
  // ✅ Utilise le hook (sauvegarde lors de validateBlock)
...
const handleEditionStartDateChange = (fieldName: string, newValue: any) => {
  // ✅ Utilise le hook avec logique de propagation aux courses
  if (racesCount > 0) {
...
#### 3. État consolidé utilisé (lignes 809-870)
...
const context: GroupedProposalContext = {
  ...
  // ✅ États depuis workingGroup
  selectedChanges: workingGroup ? {} : selectedChanges, // Vidé en mode hook
  userModifiedChanges: workingGroup?.userModifiedChanges || {},
  userModifiedRaceChanges: workingGroup?.userModifiedRaceChanges || {},
...
✅ **Plus de perte de modifications** : Le hook garde l'état consolidé  
✅ **Sauvegarde automatique** : Autosave activé (debounced)  
✅ **Validation cohérente** : Payload complet lors de la validation par blocs  
✅ **Single Source of Truth** : `workingGroup` contient tout  

### Ce qui reste (PHASE 3 - Nettoyage)

🧹 **Code legacy à supprimer** :
- `selectedChanges` (lignes 182, 234, etc.) - maintenu pour compatibilité
...
⚠️ **Non urgent** : Le système fonctionne correctement avec ces éléments en place.

---

## ✅ PHASE 2 : ProposalDetailBase (TERMINÉ)

### Fichier
`apps/dashboard/src/pages/proposals/detail/base/ProposalDetailBase.tsx`

#### Modifications effectuées (12 novembre 2025 - après-midi)
...
- `confirmEditionDateUpdate` : Utilise `updateFieldEditor` pour mettre à jour les dates d'édition

#### 3. Données consolidées depuis workingProposal (lignes 310-376)
...
const consolidatedRaceChanges: ConsolidatedRaceChange[] = useMemo(() => {
  ...
  // Fallback avec adaptation de type RaceChange[] → ConsolidatedRaceChange[]
...
**Extraction des valeurs proposées** :
```typescript
const proposedValues = useMemo(() => {
  if (workingProposal) {
    const values: Record<string, any> = {}
...
const { ... } = useBlockValidation({
  ...
  blockProposals,
  selectedChanges: proposedValues, // ✅ Depuis workingProposal
  userModifiedChanges: workingProposal ? {} : userModifiedChanges, // Vidé en mode hook
  userModifiedRaceChanges: workingProposal ? {} : userModifiedRaceChanges // Vidé en mode hook
})
```

#### 5. Context adapté (lignes 686-730)
...
✅ **Validation cohérente** : Payload complet lors de la validation par blocs  
✅ **Single Source of Truth** : `workingProposal` contient tout  
✅ **Cohérence avec GroupedProposalDetailBase** : Même architecture  

### Ce qui reste (PHASE 3 - Nettoyage)

🧹 **Code legacy à supprimer** :
- `selectedChanges` - maintenu pour compatibilité
- États manuels `userModifiedChanges`, `userModifiedRaceChanges` - redondants avec `workingProposal`
...
### Priorité 1 : Nettoyage final (PHASE 3)

**Objectif** : Supprimer complètement l'ancien système
...
3. Évaluer si `useProposalLogic` peut être supprimé (garder uniquement les utilitaires de formatage)
4. Supprimer les fallbacks de compatibilité (`workingGroup || oldState`)
...
- **PHASE 2 (GroupedProposalDetailBase)** : `docs/proposal-state-refactor/PHASE2-MIGRATION-PROGRESS.md` (à archiver)
- **État actuel** : `docs/proposal-state-refactor/STATUS-2025-11-12.md` (ce fichier)
- **Plan ProposalDetailBase** : `docs/proposal-state-refactor/PHASE2-PROPOSAL-DETAIL-BASE.md` (nouveau)

### Documentation archivée (succès passés)
...
- **Base groupée** : `apps/dashboard/src/pages/proposals/detail/base/GroupedProposalDetailBase.tsx`
...
✅ Payload complet lors de la validation  
✅ Single Source of Truth (`workingGroup`)  
✅ Sauvegarde automatique (autosave)  
✅ Code plus simple (-150 lignes de logique manuelle)  

### Objectifs restants (Nettoyage uniquement)

- [x] ProposalDetailBase migré vers `useProposalEditor` ✅
- [ ] Ancien système complètement supprimé
- [ ] `useProposalLogic` évalué/simplifié
- [ ] Tests de non-régression complets (manuels)
...
- **Date** : 2025-11-12
- **Contexte** : Après migration complète de `GroupedProposalDetailBase`, avant migration de `ProposalDetailBase`
